...
#define cLeftSide _RtN( wcSidePanel , 1 )

AddButtonAT(wcBtnClose  , _pct(10)  , _pct(2) , _pct(3)  , _pct(4) , "X" )
AddTabsA  ( wcTabs      , _Num(0)   , cMarginT  , _Rtp(-1,-15) , cRow(1.25) , cRow(1.25) , WS_CLIPCHILDREN )
AddButtonA( wcButton    , _NextCol  , _SameRow  , cMarginR , cRow(1.25) , "Build" )
AddButtonA( wcSidePanel , _Num(0)   , _NextRow0 , _pct(0)  , _BottomE(-1.1) , "Solutions" , BS_GROUPBOX )
AddRichA  ( wcLines     , cLeftSide , _TpN(wcSidePanel,0)  , _pct(1.9*(2+1)) , _BottomN(0) ,  "" , WS_DISABLED or ES_RIGHT ) 'SS_OWNERDRAW )
...
AddButtonAT( wcBtnSide  , _RtP( wcSidePanel , 1 ) , _NextRow  , _pct(4)  , cEm(1.1) , !"\x34" , BS_AUTOCHECKBOX or BS_PUSHLIKE ) '_RtP(wcSidePanel,-4)
...
AddRichA  ( wcOutput    , cLeftSide , _NextRow  , cMarginR , _BottomE(-1.1) , "" , WS_HSCROLL or WS_VSCROLL or ES_AUTOHSCROLL or ES_READONLY )
AddEditA  ( wcQuery     , cLeftSide , _SameRow  , cMarginR , _BottomE(-1.1) , "" , WS_HSCROLL or WS_VSCROLL or ES_AUTOHSCROLL )
...
SetControlsFont( wfArrows , wcBtnMinOut , wcBtnSide )
